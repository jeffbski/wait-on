...
function waitOnImpl(opts, cb) {
  var validResult = WAIT_ON_SCHEMA.validate(opts);
  ...
  var output = opts.verbose ? console.log.bind() : function() {};

  var log = opts.log ? console.log.bind() : function() {};
  ...
  valid.forEach(function(validOpt) {
    if (options[validOpt] !== undefined) {
      ...
      if (validOpt === 'httpTimeout') {
        ...
        // all other options match their keys for request
...
function createHttp$(url, options) {
  return Rx.Observable.catch(head(url, parseHttpOptions(options)), Rx.Observable.just([{ statusCode: 999 }])).map(
    function(response) {
      ...
      return {
        ...
        val: statusCode >= 200 && statusCode <= 299 ? statusCode : -1 * statusCode,
        data: response[0]
      };
    }
  );
...
function createHttpGet$(url, options) {
  return Rx.Observable.catch(get(url, parseHttpOptions(options)), Rx.Observable.just([{ statusCode: 999 }])).map(
    function(response) {
      // Why is response in array here?
      var statusCode = response[0].statusCode;
      return {
