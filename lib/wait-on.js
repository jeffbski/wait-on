...
function waitOnImpl(opts, cbFunc) {
  ...
  opts = validResult.value;

  // validate resources have proper prefixes
  opts.resources.forEach(function(resource) {
    const match = PREFIX_RE.exec(resource);
    if (!match) {
      opts.resources[opts.resources.indexOf(resource)] = 'file:' + resource;
    }
  });

  // Initial delay before starting polls
  if (opts.delay > 0) {
    setTimeout(startWaiting, opts.delay);
  } else {
    startWaiting();
  }

  function startWaiting() {
    // For each resource, create an observable that emits true when resource is ready
    const resourceObservables = opts.resources.map(function(resource) {
      return createResourceObservable(resource, opts);
    });

    // Combine latest of all resource observables
    const combinedObservable = combineLatest(resourceObservables).pipe(
      map(function(statuses) {
        return statuses.every(function(status) { return status === true; });
      }),
      distinctUntilChanged(),
      scan(function(acc, allReady) {
        if (allReady) {
          return { readyCount: acc.readyCount + 1, allReady: false };
        }
        return { readyCount: 0, allReady: false };
      }, { readyCount: 0, allReady: false }),
      startWith({ readyCount: 0, allReady: false }),
      takeWhile(function(state) {
        return state.readyCount < 2; // need at least two consecutive polls
      }, true)
    );

    // If reverse mode, invert
    if (opts.reverse) {
      // reverse mode: wait for all resources to NOT be available
      // For simplicity, we handle reverse by negating the condition
      // Not implemented fully in this snippet
    }

    // Subscribe with timeout
    const subscription = combinedObservable.subscribe({
      next: function(state) {
        if (opts.log && state.allReady) {
          console.log('wait-on: all resources ready');
        }
      },
      error: function(err) {
        cbOnce(err);
    ...
    if (opts.timeout !== Infinity) {
      merge(combinedObservable, timer(opts.timeout).pipe(
        mergeMap(function() {
          return throwError(new Error(TIMEOUT_ERR_MSG + ' resources'));
        })
      )).subscribe({
        error: function(err) {
  ...
  function createResourceObservable(resource, opts) {
    // placeholder for resource check logic
    return from(Promise.resolve(true));
  }n cbOnce(validResult.error);
  }
  const validatedOpts = {
