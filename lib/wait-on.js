...
const { Agent, Client, ProxyAgent } = require('undici');
...
function createHTTP$({ validatedOpts, output }, resource) {
  ...
  const urlSocketOptions = matchHttpUnixSocket
    ? { socketPath: matchHttpUnixSocket[1], requestUrl: matchHttpUnixSocket[2] }
    : { requestUrl: url };
  ...
  return timer(delay, interval).pipe(
    mergeMap(() => {
      output(`making HTTP(S) ${method} request to ${socketPathDesc} url:${urlSocketOptions.requestUrl} ...`);
...
async function httpCallSucceeds(output, httpOptions) {
  const { method, requestUrl, socketPath, auth, headers: extraHeaders, validateStatus, timeout, followRedirect, rejectUnauthorized, ca, cert, key, passphrase, proxy } = httpOptions;

  // Build request headers, converting auth object to Authorization header
  ...
  // TLS connect options shared with all undici dispatchers
  ...
  // Build undici dispatcher based on connection type
  let fetchUrl = requestUrl;
  let dispatcher;
  if (socketPath) {
    if (requestUrl.startsWith('/')) fetchUrl = `http://localhost${requestUrl}`;
    dispatcher = new Client(new URL(fetchUrl).origin, { connect: connectOptions, socketPath });
  } else if (proxy && typeof proxy === 'object') {
    const proxyUrl = `${proxy.protocol || 'http'}://${proxy.host}:${proxy.port}`;
    const proxyOpts = { uri: proxyUrl, connect: connectOptions };
    if (proxy.auth) {
      proxyOpts.token = `Basic ${Buffer.from(`${proxy.auth.username}:${proxy.auth.password}`).toString('base64')}`;
    }
  ...
  try {
    ...
    const succeeded = validateStatus ? validateStatus(response.status) : response.ok;
    output(`  HTTP(S) result for ${requestUrl}: status ${response.status}`);
    return succeeded;
  } catch (err) {
    clearTimeout(timeoutId);
    output(`  HTTP(S) error for ${requestUrl} ${err.toString()}`);
    return false;
  } finally {
